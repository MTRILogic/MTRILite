...
public final class Mappable<M extends Model> {
    ...
    public Mappable(final @NonNull Bundle data){
        groupListable = new Listable<>(data);
        iterateGroupList(new OnIterationListener<M>() {
            @Override
            public void onIteration(@NonNull M group) {
    ...
    public void saveToData(final @NonNull Bundle data){
    ...
    public boolean appendGroup(@NonNull M group){
        return appendGroup(group, new Listable<M>());
    }
    ...
    public boolean appendGroupList(@NonNull ArrayList<M> groupList){
        if (groupListable.appendList(groupList)){
            for (M group : groupList){
                lastChildListable = childListableMap.put(group, new Listable<M>());
            }
            return true;
        }
        lastChildListable = null;
    ...
    public boolean insertGroup(int groupPosition, @NonNull M group){
        return insertGroup(groupPosition, group, new Listable<M>());
    }
